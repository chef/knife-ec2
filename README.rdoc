...
This is the official Opscode Knife plugin for EC2. This plugin gives knife the ability to create, bootstrap, and manage EC2 instances, create hosted zones with Route53 and manage dns records.
...
Displays all hosted zones registered in AWS Route53.

== knife ec2 dns create

Creates a new hosted zone in Route53. Only --domain is required, can be set in knife configuration.

== knife ec2 dns show

Displays hosted zone information. <b>PLEASE NOTE</b> - If :zone_id is set in knife.rb and no zone id is passed to the command, it will be used to display hosted zone information.

== knife ec2 dns delete

Removes all supplied zone ids. Will display error if zone cannot be found.

== knife ec2 dns record create

Adds a new dns record. Hosted zone should be specified along with record type, ip and name.

== knife ec2 dns record delete

Removes all records from a given hosted zone, matching certain criteria.

== knife ec2 dns assign

Easily assign domains to ec2 instances, e.g. knife ec2 dns assign -I i-j63f2b96 -D www.example.com
