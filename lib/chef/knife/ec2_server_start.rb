...
class Chef
  class Knife
    class Ec2ServerStart < Knife
      ...
      banner "knife ec2 server start (options)"
      ...
      def run
        ...
        ui.info "You have asked me to start the following servers:"
        ...
        server_ids.each do |server_id|
          ui.info "Issuing start request for #{server_id}."
          connection.servers.get( server_id ).start()
        end
