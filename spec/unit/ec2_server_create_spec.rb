...
describe Chef::Knife::Ec2ServerCreate do
  before do
    ...
    @ec2_connection = mock(Fog::Compute)
    images = mock('images')
    @ec2_connection.stub!(:images).and_return images
    images.stub!(:get).with('image').and_return mock('ami',
  ...
  describe "when configuring the bootstrap process" do
    before do
      ...
      @fqdn = 'some.fqdn.local'

      @bootstrap = @knife_ec2_create.bootstrap_for_node(@new_ec2_server, @fqdn)
    end

    it "should set the bootstrap 'name argument' to the hostname of the EC2 server" do
      @bootstrap.name_args.should == [@fqdn]
    end
